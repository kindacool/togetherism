...
public class ClubController {
	...
	public String club_namecheck(@RequestParam("club_name") String cn, Model model) throws Exception {
		...
		if(result == 1) System.out.println("중복 club_name");
		if(result == -1) System.out.println("사용 가능한 club_name");
	...
	public String club_save(@RequestParam("club_image1") MultipartFile mf, ClubDTO clubdto, HttpServletRequest request,
		...
		if (filename != "") { // 중복파일
			...
			if (size > 1000000) {
				result = 1;
				model.addAttribute("result", result);
			} else if (!file[1].toLowerCase().equals("jpg") && !file[1].toLowerCase().equals("gif") && !file[1].toLowerCase().equals("png")) {
				result = 2;
				model.addAttribute("result", result);
