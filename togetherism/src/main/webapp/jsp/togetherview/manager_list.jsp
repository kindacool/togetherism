...
<script>
	$(document).ready(function(){
			$("#sidemenu").load("manager_sidemenu.jsp");
	});
</script>
<style>
	...
	<div class="col" id="sidemenu">
		...
		<div class="btn-group">
			<button type="button" class="btn text-white" style="background-color: #5e17eb;" onClick="location.href='manager_main.do' ">관리자 메인</button>
	  		...
	  		<button type="button" class="btn text-white" style="background-color: #5e17eb;" onClick="location.href='notice_list.do' ">공지사항 작성</button>
		</div>
	</div>
	</div>
</div>
...
<div align="center">
	...
	<c:if test="${ nowMember > 0 }">
		...
		<!-- 최신 페이지 -->
		<a href ="manager_list.do?page=1" id="linkstyle"> 처음 </a>
		
		<!-- 이전 블럭 이동 -->
		<c:if test="${startPage > 10 }">
			<a href="manager_list.do?page=${startPage-10 }" id="linkstyle"> << </a>
		</c:if>
		...
		<c:forEach var="i" begin="${startPage }" end="${endPage }">
			...
			<!-- 현재 페이지가 아닐 때 -->
			<c:if test="${i != page }">
				<a href="manager_list.do?page=${i }" id="linkstyle">${i }</a>
			</c:if>
		</c:forEach>
		...
		<!-- 마지막 페이지 -->
		<a href="manager_list.do?page=${pageCount }" id="linkstyle"> 끝 </a>
