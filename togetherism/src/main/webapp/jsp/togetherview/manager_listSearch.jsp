...
<!-- 검색창 -->
<div align="center">
<form method=post action="manager_listSearch.do">
		...
		<tr>
			<td align="right" style="width: 450px">
				<select class="form-select form-select-sm" aria-label=".form-select-sm example" name="search" id="search" style="width:110px">
					<option value="member_nickname" <c:if test="${search eq 'member_nickname' }"> selected</c:if> >닉네임</option>
					<option value="member_email" <c:if test="${search eq 'member_email' }"> selected</c:if> >이메일</option>
					<option value="reportcount" <c:if test="${search eq 'reportcount' }"> selected</c:if> >신고횟수</option>
				</select>
			</td>
			<td align="center" style="width: 200px">
				<input type="search" class="form-control form-control-sm" name="keyword" id="keyword" value="${keyword }" style="width:200px">
			</td>
